...
//@ts-ignore
const tree: { children: Images; } = dirTree(path.join(__dirname, "../src/assets/goats"), { exclude: /\.json$|.DS_Store/ });
console.debug(tree);
...
for (const goat of tree.children) {
  ...
  map[goat.name] = `data:image/${path.extname(image.name).replace('.', '')};base64,${fs.readFileSync(image.path, 'base64')}`;
}
